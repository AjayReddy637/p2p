...
def render_bg_button_sidebar():
    """
    BG button: white circle. Click opens st.popover with colour picker directly —
    no hidden swatch button. Colour applies immediately to dashboard background.
    """
    current_bg = st.session_state.get("bg_color", "#ffffff")

    # ── CSS for the circle BG button ──────────────────────────────────────────
    st.markdown("""
<style>
/* White circle BG button — the popover trigger */
...
/* Inside popover: hide color picker label, clean layout */
div[data-testid="stPopover"] div[data-testid="stColorPicker"] label {
...
""", unsafe_allow_html=True)
    ...
    # ── st.popover: opens a popup directly when clicked — no extra button ─────
    with st.popover("BG", use_container_width=False):
        safe_val = current_bg if (
            current_bg.startswith("#") and len(current_bg) in (4, 7)
        ) else "#ffffff"
        picked = st.color_picker(
            "Background", value=safe_val,
...
""", unsafe_allow_html=True)
    ...
    # ── Outer container using st.container so widgets render INSIDE it ───────
    st.markdown(
