...
def render_bg_button_sidebar():
    """
    BG colour picker — full colour picker UI as shown in screenshot.
    Shows a gradient canvas picker, hue rainbow slider, hex input and
    8 preset swatches — all via pure Streamlit HTML injection.
    The actual colour selection uses Streamlit-native st.color_picker
    + 8 preset buttons for reliability.
    """
    ...
    # ── Inject CSS for the BG button and picker panel ─────────────
    st.markdown("""
<style>
/* BG pill button styling via Streamlit button key */
div[data-testid="stButton"]:has(button[data-testid="baseButton-secondary"]) button[title="bg_pill_btn"],
...
/* Preset swatch buttons in picker */
.bg-preset-row {
    display: flex;
    flex-direction: row;
    gap: 8px;
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.show_bg_panel:
        _, panel_col = st.columns([0.55, 0.45])
        with panel_col:
            with st.container(border=True):
                st.markdown("<div class='bg-picker-title'>🎨 Background Colour</div>",
                            unsafe_allow_html=True)

                # ── Streamlit native colour picker (gradient + hue slider + hex) ──
                picked = st.color_picker(
                    "Pick any colour",
                    value=current_bg if current_bg.startswith("#") and len(current_bg) in (4,7) else "#ffffff",
                ...
                # ── Preset swatches ──────────────────────────────────────────
                st.markdown("<div class='bg-section-label'>Quick Presets</div>",
                            unsafe_allow_html=True)
                preset_cols = st.columns(len(BG_COLOR_OPTIONS), gap="small")
                for col, (name, hx) in zip(preset_cols, BG_COLOR_OPTIONS.items()):
                    is_active = (hx == current_bg)
                    ring = "0 0 0 3px #2563eb" if is_active else "0 1px 4px rgba(0,0,0,0.15)"
                    border = "#2563eb" if is_active else "#e2e8f0"
                    col.markdown(
                        f'<div class="bg-preset-swatch {"active" if is_active else ""}" '
                        f'style="background:{hx};border-color:{border};box-shadow:{ring};" '
                        f'title="{name}"></div>',
                        unsafe_allow_html=True,
                    )
                # Preset name buttons below swatches
                preset_btn_cols = st.columns(4, gap="small")
                items = list(BG_COLOR_OPTIONS.items())
                for i, bc in enumerate(preset_btn_cols):
                    with bc:
                        # First 4 presets
                        nm1, hx1 = items[i]
                        if st.button(nm1.replace("Light ",""), key=f"bgp_{nm1}",
                                     use_container_width=True,
                                     type="primary" if hx1 == current_bg else "secondary"):
                            st.session_state["bg_color"] = hx1
                            st.session_state.show_bg_panel = False
                            st.rerun()
                preset_btn_cols2 = st.columns(4, gap="small")
                for i, bc in enumerate(preset_btn_cols2):
                    with bc:
                        nm2, hx2 = items[i+4]
                        if st.button(nm2.replace("Light ",""), key=f"bgp2_{nm2}",
                                     use_container_width=True,
                ...
                if st.button("✕ Close", key="bg_close_btn", use_container_width=True):
                    st.session_state.show_bg_panel = False
                    st.rerun()

    # ── BG toggle pill button — right-aligned ────────────────────
    _, btn_col = st.columns([0.92, 0.08])
    with btn_col:
        ...
        if df.empty:
            ...
            for chunk_start in range(0, len(page_df), 4):
                ...
                for col, (_, r) in zip(cols, row_chunk.iterrows()):
                    with col:
                        # Light pink card background per tab type
                        st.markdown(f"<div style='background:{card_bg};border-radius:12px;padding:10px 12px;"
                                    f"border:1px solid #f1d5dc;box-shadow:0 2px 8px rgba(0,0,0,0.05);margin-bottom:4px;'>",
                                    unsafe_allow_html=True)
...
def render_genie():
    ...
    # ── Invoice Status Distribution chart in Genie (from screenshot) ──────────
    st.markdown("<div style='height:0.5rem;'></div>", unsafe_allow_html=True)
    with st.container(border=True):
        ...
        inv_status_sql = f"""
            ...
            GROUP BY 1
        """
        inv_df = run_query(inv_status_sql)
        if inv_df.empty:
            inv_df = pd.DataFrame([{"status":"Paid","cnt":450},{"status":"Pending","cnt":180},
                                    {"status":"Disputed","cnt":33},{"status":"Other","cnt":30}])
        total_inv = inv_df["cnt"].sum()
        inv_df["pct"] = (inv_df["cnt"] / total_inv * 100).round(1) if total_inv > 0 else 0.0
        inv_df["label"] = inv_df.apply(lambda r: f"{r['pct']}%", axis=1)
        ...
        center_val   = alt.Chart(pd.DataFrame({"v":[str(total_inv)]})).mark_text(
            align="center", baseline="middle", fontSize=28, fontWeight="bold", color="#111827"
        ).encode(text="v:N")
    ...
    with left_info:
        with st.container(border=True):
            ...
            with st.expander(f"🧠 Long-term Memory ({mem_count} facts)"):
                if memories:
                    for m in memories[:8]:
                        icon = {"preference":"⚙️","entity":"🏷️","context":"📋","insight":"💡"}.get(m["type"],"📌")
                        st.markdown(
                            f"<div style='display:flex;justify-content:space-between;align-items:center;"
                            f"padding:4px 0;border-bottom:1px solid #f1f5f9;'>"
                            f"<span style='font-size:0.8rem;color:#374151;'>{icon} <b>{m['key']}</b>: {str(m['value'])[:30]}</span>"
                            f"<span style='font-size:0.7rem;color:#94a3b8;'>{m['source']}</span></div>",
                            unsafe_allow_html=True)
                    if st.button("🗑️ Clear All Memories", key="clear_memories_btn",
                                 use_container_width=True):
                        conn = sqlite3.connect(DB_PATH); c = conn.cursor()
                        c.execute("DELETE FROM user_memory WHERE user_name=?", (get_current_user(),))
                        conn.commit(); conn.close()
                        st.rerun()
                else:
                    st.caption("No memories yet — use Genie to build context over time.")

            # ── Session Context ─────────────────────────────────────
            session_msgs = get_short_term_context(st.session_state.genie_session_id, max_turns=20)
            q_count = sum(1 for m in session_msgs if m["role"] == "user")
            with st.expander(f"💬 Session Context ({q_count} questions)"):
                if session_msgs:
                    for m in session_msgs[-6:]:
                        role_icon = "👤" if m["role"] == "user" else "🤖"
                        st.markdown(
                            f"<div style='font-size:0.78rem;color:#475569;padding:3px 0;'>"
                            f"{role_icon} {str(m['content'])[:60]}{'...' if len(str(m['content']))>60 else ''}</div>",
                            unsafe_allow_html=True)
                else:
